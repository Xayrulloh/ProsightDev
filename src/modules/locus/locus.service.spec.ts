...
describe('LocusService', () => {
  ...
  describe('role-based access', () => {
    ...
    it('limited subquery uses the allowlist region IDs', async () => {
      await service.findAll(asQuery(baseQuery), limited);
      const allowedCall = memberQb.andWhere.mock.calls.find(
        (c) => (c[1] as Record<string, unknown>)?.allowed,
      );
      expect(allowedCall).toBeDefined();
      expect((allowedCall![1] as { allowed: number[] }).allowed).toEqual([
        ...LIMITED_ROLE_ALLOWED_REGION_IDS,
      ]);
    });
    ...
    it('normal + sideload throws ForbiddenException', async () => {
      await expect(
        service.findAll(asQuery({ ...baseQuery, sideload: 'locusMembers' }), normal),
      ).rejects.toBeInstanceOf(ForbiddenException);
    });

    it('admin + sideload does NOT throw', async () => {
      await expect(
        service.findAll(asQuery({ ...baseQuery, sideload: 'locusMembers' }), admin),
      ).resolves.toEqual([]);
    });

    it('limited + sideload does NOT throw (still restricted by allowlist)', async () => {
      await expect(
        service.findAll(asQuery({ ...baseQuery, sideload: 'locusMembers' }), limited),
      ).resolves.toEqual([]);
    });
  });
  ...
  describe('filtering', () => {
    ...
    it('regionId filter triggers EXISTS subquery for admin', async () => {
      await service.findAll(
        asQuery({ ...baseQuery, regionId: [1, 2] }),
        admin,
      );
      expect(andWhereStrings().some((s) => s.startsWith('EXISTS'))).toBe(true);
    ...
    it('limited + user regionId: BOTH clauses applied to subquery', async () => {
      await service.findAll(asQuery({ ...baseQuery, regionId: [999] }), limited);
      const subCalls = memberQb.andWhere.mock.calls;
      expect(
        subCalls.some((c) => (c[1] as Record<string, unknown>)?.rids),
      ).toBe(true);
      expect(
        subCalls.some((c) => (c[1] as Record<string, unknown>)?.allowed),
      ).toBe(true);
    });
